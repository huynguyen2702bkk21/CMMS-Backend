...
namespace WebAPICHATest.Api.Application.Queries.Equipments
{
    public class EquipmentByIdQueryHandler : IRequestHandler<EquipmentByIdQuery, QueryResult<EquipmentViewModel>>
    {
        ...
        public async Task<QueryResult<EquipmentViewModel>> Handle(EquipmentByIdQuery request, CancellationToken cancellationToken)
        {
            ...
            foreach (var equipment in requests)
            {
            ...
            foreach (var equipment in requests)
            {
                ...
                equipmentGet.ScheduleWorkingTimes = null;
                if (equipment.ScheduleWorkingTimes != null)
                {
            ...
            //var result = await _context.Equipments.AsNoTracking().FirstOrDefaultAsync(c => c.EquipmentId == request.EquipmentId);

            //return _mapper.Map<Equipment, EquipmentViewModel>(result);
        }
    }
}
